...
export default function AdminDashboardScreen() {
  ...
  const renderAppointment = ({ item }: { item: any }) => (
    <View className="bg-slate-800 rounded-2xl p-4 mb-4 border border-slate-700 shadow-sm">
      ...
      <View className="bg-slate-900 rounded-xl p-3 mt-2">
        <Text className="text-slate-300 text-sm">Tür: {item.pet_details?.species === 'Dog' ? 'Köpek' : 'Kedi'} | Cins: {item.pet_details?.breed}</Text>
        <Text className="text-slate-400 text-xs mt-1">Yaş: {item.pet_details?.age} {item.service_type === 'Hotel' ? `| Tarih: ${item.pet_details?.checkInDate} - ${item.pet_details?.checkOutDate}` : `| Kilo: ${item.pet_details?.weight}`}</Text>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-900">
      <View className="px-6 pt-4 pb-4 flex-row justify-between items-center border-b border-slate-800">
        <View>
          <Text className="text-2xl font-bold text-white">Yönetim Paneli</Text>
          <Text className="text-slate-400 mt-1">Takvim ve Gelen Kutusu</Text>
        </View>
        <TouchableOpacity onPress={handleLogout} className="bg-slate-800 p-3 rounded-full">
      ...
      <View className="px-6 py-4">
        <View className="flex-row bg-slate-800 rounded-xl p-1">
          ...
          >
            <Text className={`font-semibold ${activeTab === 'Calendar' ? 'text-white' : 'text-slate-400'}`}>Takvim</Text>
          </TouchableOpacity>
          <TouchableOpacity 
            className={`flex-1 py-2 items-center rounded-lg ${activeTab === 'Inbox' ? 'bg-slate-700 shadow-sm' : ''}`}
            onPress={() => setActiveTab('Inbox')}
          >
            <Text className={`font-semibold ${activeTab === 'Inbox' ? 'text-white' : 'text-slate-400'}`}>Gelen Kutusu</Text>
          </TouchableOpacity>
        </View>
      </View>
          ...
          <View className="flex-1 px-2">
            {loading ? (
              ...
              <View className="items-center justify-center mt-10">
                ...
                <Text className="text-slate-400 mt-4">Henüz randevu bulunmuyor.</Text>
              </View>
            )}
          </View>
        )}
